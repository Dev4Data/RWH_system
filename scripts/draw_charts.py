...
def chart_wspd_wdir(df_in: pd.DataFrame, filename):
    """wind speed vs wind direction chart total"""
    df_tmp = df_in.reset_index(drop=False)
    df_wdir_wspd \
    ...
    fig1.savefig(f"{diagram_path}heatmap_{filename}.png")
    plt.close()


def chart_wspd_wdir_monthly(df_in: pd.DataFrame, filename):
    ...
    for m in range(1, 12):
        ...
        plt.xlabel('wind direction °')
        plt.ylabel('wind speed kph')
        fig1.savefig(f"{diagram_path}monthly/heatmap_{filename}_" +str(m)+ ".png")
        plt.close()
        del df_tmp_month
        del df_wdir_wspd


def chart_windrose(df_in: pd.DataFrame):
    ...
    fig3.add_axes(ax3)
    ax3.bar(df_wind["wdir"].astype(float), df_wind["wspd"].astype(float), normed=True, opening=1)
    ax3.set_title('wind direction with speed')
    plt.savefig(f"{diagram_path}windrose_wdir.png")
    plt.close()


def chart_windrose_yearly(df_in: pd.DataFrame):
...
#    df_tmp.reset_index(inplace=True)
    ...
    for y in years:
        ...
        year = str(y)
        ax3.set_title(f"wind direction with speed for year {year}")
        plt.savefig(f"{diagram_path}yearly/windrose_wdir{year}.png")
        plt.close()


def chart_ym_heatmap(df_in: pd.DataFrame, fields, filename, title, xlabel, ylabel, cmap):
    ...
    df_pivot = df_tmp.pivot(index=fields[0], columns=fields[1], values=fields[2])
    fig2, (ax2) = plt.subplots(1, 1, figsize=(20, 12))
    fig2.suptitle(title)
    sns.heatmap(df_pivot, ax=ax2, cmap=cmap, annot=True, fmt='g')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    fig2.savefig(f"{diagram_path}heatmap_{filename}.png")
    plt.close()
    del df_pivot


def chart_freq(df_in: pd.DataFrame, filename, title, xlabel, ylabel, bins=50):
    ...
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.savefig(f"{diagram_path}distplot_{filename}.png")
    plt.close()


def chart_precip_sum_ym(df_in: pd.DataFrame, file_name):
    ...
    ax1.set(xticks=df_tmp.month.values)
    ax1.set_xlabel("Year-Month")
    ax1.set_ylabel("recipation in mm", labelpad=10)
    ax1.set_title("Year-Month precipation sum")
    fig1.savefig(f"{diagram_path}lineplot{file_name}.png")
    plt.close()


def precip_sum_yw(df_in: pd.DataFrame):
    df_tmp = df_in.reset_index(drop=False)
    df_tmp = df_tmp[['year', 'week', 'precip']]
    df_tmp['week_int'] = df_tmp['week'].astype(np.int64)
...
def chart_precip_sum_yw(df_in: pd.DataFrame):
    ...
    df_tmp = df_in.reset_index(drop=False)
#    df_tmp.reset_index(inplace=True)
    df_tmp = df_tmp[df_tmp['year'] >= datetime.datetime.today().year -7].astype(str)
    df_tmp_grp = precip_sum_yw(df_tmp)
    fig1, (ax1) = plt.subplots(1, 1, figsize=(15, 8))
    fig1.suptitle('Year-Week charts')
    sns.set(style="darkgrid"
            , rc={"lines.linewidth": 1, "legend.fontsize": 8, "legend.loc": 'upper center'})
    sns.lineplot(data=df_tmp_grp, sort=True,
                 x='week', y="precip_sum",
                 hue="year",
    ...
    ax1.set(xticks=df_tmp.week.values)
    ax1.set_xlabel("Year-week")
    ax1.set_ylabel("recipation in mm", labelpad=10)
    ax1.set_title("Year-week precipation sum")
    fig1.savefig(f"{diagram_path}lineplot_yw_precip_sum.png")
    plt.close()


def chart_precip_sum_to_h_yearly(df_in: pd.DataFrame, df_tmp_grp: pd.DataFrame):
    ...
    for y in years:
        ...
        year = str(y)
        g1.savefig(f"{diagram_path}yearly/pairplot_precip_sum_h{year}.png")
        plt.close()


def chart_precip_month(df_tmp: pd.DataFrame, filename):
...
def chart_df_totals(df_tmp_grp: pd.DataFrame):
    ...
    # tab.set_fontsize(8)
    # tab.scale(1.2, 1.2)
    plt.savefig(f"{diagram_path}df_totals.png", transparent=True)
    plt.close()
