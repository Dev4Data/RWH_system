...
def chart_freq(df_tmp, filename, title, xlabel, ylabel, bins=50):
    ...
    fig1, (ax1) = plt.subplots(1, 1, figsize=(20, 12))
    fig1.suptitle(title)
    ax1 = sns.distplot(df_tmp, bins=bins)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    fig1.savefig("./diagrams/distplot_" + filename + ".png")
...
def chart_precip_sum_to_h(df_tmp, df_tmp_grp):
    # Create figure and plot space
...
tab = pd.plotting.table(ax, df_total, loc='upper right')
